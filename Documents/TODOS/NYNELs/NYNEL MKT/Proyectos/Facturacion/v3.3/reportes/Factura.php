...
function addCadreEurosFrancs($impuesto, $nombretrib)
{
    ...
    $r1  = $this->w - 70;
    $r2  = $r1 + 60;
    $y1  = $this->h - 185;  // Manteniendo y-position para página completa
...
{
    // MODERNO: Formato de 9 líneas para segunda copia
    $r1  = $this->w - 70;
    $r2  = $r1 + 60;
    $y1  = $this->h - 39;  // Manteniendo y-position para segunda copia
    $y2  = $y1+52; // MODERNO: 52 unidades para 9 líneas
    $this->RoundedRect($r1, $y1, ($r2 - $r1), ($y2-$y1), 2.5, 'D');
    $this->Line( $r1+35,  $y1, $r1+35, $y2);
...
function addTVAs2($total, $moneda, $tdescuento, $ipagado, $saldo, $icbper, $igv, $opGravada, $opExonerado)
{
    // MODERNO: 9 parámetros para segunda copia
    $this->SetFont('Arial','',8);
    $re  = $this->w - 30;
    $y1  = $this->h - 39;  // Manteniendo y-position para segunda copia
    $this->SetFont( "Arial", "B", 10);

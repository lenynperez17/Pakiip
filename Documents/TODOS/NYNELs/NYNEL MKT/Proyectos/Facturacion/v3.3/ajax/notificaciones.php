		...
		$rspta=$notificaciones->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				...
 				"5" => ($reg->estado) ? '<button class="btn btn-icon btn-sm btn-info" onclick="mostrar(' . $reg->idnotificacion . ')"><i class="ri-edit-line"></i></button>' .
				' <button class="btn btn-icon btn-sm btn-danger" onclick="desactivar(' . $reg->idnotificacion . ')"><i class="ri-delete-bin-line"></i></button>' :
				'<button class="btn btn-icon btn-sm btn-info" onclick="mostrar(' . $reg->idnotificacion . ')"><i class="ri-edit-line"></i></button>' .
				' <button class="btn btn-icon btn-sm btn-success" onclick="activar(' . $reg->idnotificacion . ')"><i class="ri-check-double-line"></i></button>'
 				);
 		}
 		$results = array(

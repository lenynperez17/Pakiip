...
switch ($_GET["op"]) {
    case 'guardaryeditarnc':
        if (empty($idnota)) {
            ...
            // ============================================================================
            // VALIDACIÓN BACKEND CRÍTICA SUNAT: Nota de Crédito según tipo de comprobante
            // ============================================================================
            // El tipo de comprobante modificado se recibe en $tipodoc_mod o $_GET['tipodo']
            $tipoComprobanteModificado = isset($_GET['tipodo']) ? $_GET['tipodo'] : $tipodoc_mod;

            // Validar según el tipo de comprobante que se está acreditando
            if ($tipoComprobanteModificado == "01") {
                // NC para FACTURA → Cliente DEBE tener RUC (tipo_doc_ide = 6)
                if ($tipo_doc_ide != "6") {
                    echo json_encode([
                        "status" => "error",
                        "message" => "ERROR DE VALIDACIÓN SUNAT: Las Notas de Crédito para Facturas solo pueden emitirse a clientes con RUC. " .
                                    "El cliente seleccionado tiene tipo de documento: " . $tipo_doc_ide . ". " .
                                    "Por favor, verifique el tipo de documento del cliente."
                    ]);
                    exit();
                }
                ...
                if (strlen($numero_documento) != 11 || !ctype_digit($numero_documento)) {
                    echo json_encode([
                        ...
                        "message" => "ERROR DE VALIDACIÓN: El RUC debe tener exactamente 11 dígitos numéricos. " .
                                    "RUC actual: " . $numero_documento
                    ]);
                    exit();
                }
            } elseif ($tipoComprobanteModificado == "03") {
                // NC para BOLETA → Cliente puede tener DNI, CE, Pasaporte o RUC
                // Validar que tenga algún tipo de documento válido
                $tiposValidos = ["1", "4", "6", "7"]; // DNI, CE, RUC, Pasaporte
                if (!in_array($tipo_doc_ide, $tiposValidos)) {
                    echo json_encode([
                        "status" => "error",
                        "message" => "ERROR DE VALIDACIÓN: Tipo de documento no válido para Nota de Crédito de Boleta. " .
                                    "Tipo de documento actual: " . $tipo_doc_ide
                    ]);
                    exit();
                }
            }
            ...
            if ($tiponotaC == '7') { //Si es nota de credito por item.
                ...
                // Si la nota de crédito se creó exitosamente, marcar el comprobante original como anulado
                if ($rspta && is_numeric($rspta)) {
                    // Llamar a ActualizarEstado para cambiar el estado del comprobante original a '0' (ANULADO)
                    $notacd->ActualizarEstado($rspta, '1', $idcomprobante, $tipodocu, $numero_comprobante);
                }

...
class Factura
{
  ...
  // Método público para obtener el último error MySQL capturado
...
//             $handle=fopen($path, "a");
//            fwrite($handle, $fecha."|".$fechabaja[$i]."|".$tipocomp[$i]."|".$numeroc[$i]."|".$comen[$i]."|\r\n");
//            fclose($handle);
...
//**************************************************************************
  ...
  public function mostrarxml($idfactura, $idempresa)
  {
    ...
    $query = "SELECT
     ...
     WHERE idfactura = ? AND f.estado IN('1','4','5')
     ORDER BY numerodoc";
    ...
    if ($result) {
      // Procesar directamente en while (SIN for loop - Bug corregido)
  ...
  public function mostrarrpta($idfactura, $idempresa)
  {
    ...
    $query = "SELECT
     ...
     factura f INNER JOIN persona p ON f.idcliente = p.idpersona
     INNER JOIN empresa e ON f.idempresa = e.idempresa
     WHERE idfactura = ? AND f.estado IN('5','4')
     ORDER BY numerodoc";
  ...
  public function generarxml($idfactura, $idempresa)
  {
    ...
    // QUERY 1: Datos de cabecera con prepared statement
    $query = "SELECT
     ...
     INNER JOIN empresa e ON f.idempresa = e.idempresa
     WHERE f.idfactura = ? AND f.estado IN('1','4')
     ORDER BY numerodoc";

    $stmt1 = $conexion->prepare($query);
    $stmt1->bind_param("i", $idfactura);
    $stmt1->execute();
    $result = $stmt1->get_result();
    $stmt1->close();

    // QUERY 2: Cuotas con prepared statement
    $querycuotas = "SELECT
     ...
     WHERE cu.idcomprobante = ? AND cu.tipocomprobante = '01'";

    $stmt2 = $conexion->prepare($querycuotas);
    $stmt2->bind_param("i", $idfactura);
    $stmt2->execute();
    $resultcuotas = $stmt2->get_result();
    $stmt2->close();

    // QUERY 3: Detalle factura con prepared statement
    $querydetfac = "SELECT
       f.tipo_documento_07 AS tipocomp,
       f.numeracion_08 AS numerodoc,
       ...
       df.afectacion_igv_item_16_4 AS codtrib,
    ...
    $stmt3 = $conexion->prepare($querydetfac);
    $stmt3->bind_param("i", $idfactura);
    $stmt3->execute();
    $resultf = $stmt3->get_result();
    $stmt3->close();
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      if ($formapago == 'Contado') {
        ...
        // Procesar cuotas directamente en while (SIN for loop - BUG CORREGIDO)
        while ($rowb = $resultcuotas->fetch_assoc()) {
          $ncuota = $rowb["ncuota"];
          $montocuota = $rowb["montocuota"];
          $fechacuota = $rowb["fechacuota"];
          $formapagocre = $rowb["formapago"];
          $monedaf = $rowb["monedaf"];

          $facturaXML .= '<cac:PaymentTerms>
              <cbc:ID>FormaPago</cbc:ID>
              <cbc:PaymentMeansID>Cuota' . $ncuota . '</cbc:PaymentMeansID>
              <cbc:Amount currencyID="' . $monedaf . '">' . $montocuota . '</cbc:Amount>
    ...
    // Procesar items directamente en while (SIN for loop - BUG CORREGIDO en líneas 1510-1662 del original)
    while ($rowf = $resultf->fetch_assoc()) {
      // Extraer todos los campos como variables escalares
      $codigo = $rowf["codigo"];
      ...
      // Determinar IGV según código de tributo
      if ($codtrib == '9997') {
        $igv_ = "0";
      } else {
        $igv_ = $configE->igv;
...
<!--<cac:TaxSubtotal>
      ...
      // Agregar tributo ICBPER si corresponde
      if ($codigo == "ICBPER") {
        $facturaXML .= '
              <cac:TaxSubtotal>
  ...
  public function generarxmlEA($ano, $mes, $dia, $idfactura, $estado, $check, $idempresa)
  {
    ...
    if ($estado == '1' && $estado == '4' || $check == 'true') {
      ...
      $query = "SELECT
        ...
        INNER JOIN persona p ON f.idcliente = p.idpersona
        INNER JOIN empresa e ON f.idempresa = e.idempresa
        WHERE YEAR(f.fecha_emision_01) = ? AND MONTH(f.fecha_emision_01) = ? AND DAY(f.fecha_emision_01) = ?
          AND f.estado = ? AND f.idfactura = ? AND f.estado != '3'
        ORDER BY numerodoc";

      $stmt1 = $conexion->prepare($query);
      if (!$stmt1) {
        error_log("Error preparando SELECT cabecera en generarxmlEA(): " . $conexion->error);
        return false;
      }
      $stmt1->bind_param("ssssi", $ano, $mes, $dia, $estado, $idfactura);
      if (!$stmt1->execute()) {
        error_log("Error ejecutando SELECT cabecera en generarxmlEA(): " . $stmt1->error);
        $stmt1->close();
        return false;
      }
      $result = $stmt1->get_result();
      ...
      // ============= QUERY 2: DETALLE FACTURA CON PREPARED STATEMENT =============
      $querydetfac = "SELECT
        f.tipo_documento_07 AS tipocomp,
        f.numeracion_08 AS numerodoc,
        df.cantidad_item_12 AS cantidad,
      ...
      $stmt2 = $conexion->prepare($querydetfac);
      if (!$stmt2) {
        error_log("Error preparando SELECT detalle en generarxmlEA(): " . $conexion->error);
        return false;
      }
      $stmt2->bind_param("ssssi", $ano, $mes, $dia, $estado, $idfactura);
      if (!$stmt2->execute()) {
        error_log("Error ejecutando SELECT detalle en generarxmlEA(): " . $stmt2->error);
        $stmt2->close();
        return false;
      }
      $resultf = $stmt2->get_result();
      $stmt2->close();
      ...
      // Parámetros de salida (procesados directamente en while - SIN arrays ni for loop)
      $con = 0;
      $icbper = "";
      $igv_ = "0";

      // Procesar cabecera directamente en while (SIN for loop - BUG CORREGIDO)
      while ($row = $result->fetch_assoc()) {
          // Extraer todos los campos como variables escalares
          $fecha = $row["fecha"];
          $serie = $row["serie"];
          $tipodocu = $row["tipodocuCliente"];
          $numdocu = $row["numero_documento"];
          $rasoc = $row["razon_social"];
          $moneda = $row["tipo_moneda_28"];
          $subtotal = $row["subtotal"];
          $igv = $row["igv"];
          $total = $row["total"];
          ...
          $ubigueo = $datose->ubigueo;
          $opera = $row["opera"];
          $codigotrib = $row["codigotrib"];
          $nombretrib = $row["nombretrib"];
          $codigointtrib = $row["codigointtrib"];
          $formapago = $row["formapago"];
          $montofpago = $row["montofpago"];
          $monedafpago = $row["monedafpago"];
          $ccuotas = $row["ccuotas"];
      ...
      // Procesar items directamente en while (SIN for loop - Bug corregido)
      while ($rowf = $resultf->fetch_assoc()) {
          $codigo = $rowf["codigo"];
          $cantidad = $rowf["cantidad"];
          $descripcion = $rowf["descripcion"];
  ...
  public function regenerarxml($idfactura, $idempresa)
  {
    ...
    $query = "SELECT
      ...
      INNER JOIN persona p ON f.idcliente = p.idpersona
      INNER JOIN empresa e ON f.idempresa = e.idempresa
      WHERE f.idfactura = ? AND f.estado IN('1','4','3','5')
      ORDER BY numerodoc";

    $stmt1 = $conexion->prepare($query);
    if (!$stmt1) {
      error_log("Error preparando SELECT cabecera en regenerarxml(): " . $conexion->error);
      return false;
    }
    $stmt1->bind_param("i", $idfactura);
    if (!$stmt1->execute()) {
      error_log("Error ejecutando SELECT cabecera en regenerarxml(): " . $stmt1->error);
      $stmt1->close();
      return false;
    }
    $result = $stmt1->get_result();
    ...
    $stmt2 = $conexion->prepare($querycuotas);
    if (!$stmt2) {
      error_log("Error preparando SELECT cuotas en regenerarxml(): " . $conexion->error);
      return false;
    }
    $stmt2->bind_param("i", $idfactura);
    if (!$stmt2->execute()) {
      error_log("Error ejecutando SELECT cuotas en regenerarxml(): " . $stmt2->error);
      $stmt2->close();
      return false;
    }
    $resultcuotas = $stmt2->get_result();
    ...
    $stmt3 = $conexion->prepare($querydetfac);
    if (!$stmt3) {
      error_log("Error preparando SELECT detalle en regenerarxml(): " . $conexion->error);
      return false;
    }
    $stmt3->bind_param("i", $idfactura);
    if (!$stmt3->execute()) {
      error_log("Error ejecutando SELECT detalle en regenerarxml(): " . $stmt3->error);
      $stmt3->close();
      return false;
    }
    $resultf = $stmt3->get_result();
...
<!--<cac:TaxSubtotal>
  ...
  public function enviarxmlSUNAT($idfactura, $idempresa)
  {
    ...
    $sqlsendmail = "SELECT
        ...
        INNER JOIN empresa e ON f.idempresa = e.idempresa
        WHERE
        f.idfactura = ? AND e.idempresa = ?";

    $stmt = $conexion->prepare($sqlsendmail);
...
as tabla where year(fecha)='$ano' and month(fecha)='$mes' and day(fecha)='$dia' and idempresa='$idempresa'";
  ...
  public function duplicar($idfactura)
  {
    ...
    // Query 1 con prepared statement
    $seriebol = "SELECT LEFT(numeracion_08,4) AS serie FROM factura WHERE idfactura = ?";
    $stmt1 = $conexion->prepare($seriebol);
    $stmt1->bind_param("i", $idfactura);
    $stmt1->execute();
    $buscaserie = $stmt1->get_result();
    $stmt1->close();

    $serie = "";
    // Procesar directamente en while (SIN for loop - Bug corregido)
    while ($row = $buscaserie->fetch_assoc()) {
        $serie = $row["serie"];
    }

    // Query 2 con prepared statement
    $buscanumero = "SELECT numero FROM numeracion WHERE serie = ?";
    $stmt2 = $conexion->prepare($buscanumero);
    $stmt2->bind_param("s", $serie);
    $stmt2->execute();
    $numeroobt = $stmt2->get_result();
    $stmt2->close();

    $nnumero = 0;
    // Procesar directamente en while (SIN for loop - Bug corregido)
    while ($row = $numeroobt->fetch_assoc()) {
        $nnumero = $row["numero"];
    }
  ...
  public function solofirma($ano, $mes, $dia, $idfactura, $estado, $check, $idempresa)
  {
    ...
    if ($estado == '1' && $estado == '4' || $check == 'true') {
      ...
      // Query cabecera con prepared statement
      $query = "SELECT
        DATE_FORMAT(f.fecha_emision_01, '%Y-%m-%d') AS fecha,
        RIGHT(SUBSTRING_INDEX(f.numeracion_08,'-',1),1) AS serie,
        DATE_FORMAT(f.fecha_emision_01, '%H:%i:%s') AS hora,
      ...
      INNER JOIN persona p ON f.idcliente = p.idpersona
      INNER JOIN empresa e ON f.idempresa = e.idempresa
      WHERE YEAR(f.fecha_emision_01) = ?
        AND MONTH(f.fecha_emision_01) = ?
        AND DAY(f.fecha_emision_01) = ?
        AND f.estado = ?
        AND f.idfactura = ?
        AND f.estado != '3'
      ORDER BY numerodoc";
      ...
      $stmt = $conexion->prepare($query);
      if (!$stmt) {
        error_log("Error preparando SELECT cabecera en solofirma(): " . $conexion->error);
        return false;
      }
      $stmt->bind_param("iiisi", $ano, $mes, $dia, $estado, $idfactura);
      if (!$stmt->execute()) {
        error_log("Error ejecutando SELECT cabecera en solofirma(): " . $stmt->error);
        $stmt->close();
        return false;
      }
      $result = $stmt->get_result();
      ...
      // Query detalle con prepared statement
      $querydetfac = "SELECT
        f.tipo_documento_07 AS tipocomp,
        f.numeracion_08 AS numerodoc,
        df.cantidad_item_12 AS cantidad,
      ...
      $stmtf = $conexion->prepare($querydetfac);
      if (!$stmtf) {
        error_log("Error preparando SELECT detalle en solofirma(): " . $conexion->error);
        return false;
      }
      $stmtf->bind_param("iiisi", $ano, $mes, $dia, $estado, $idfactura);
      if (!$stmtf->execute()) {
        error_log("Error ejecutando SELECT detalle en solofirma(): " . $stmtf->error);
        $stmtf->close();
        return false;
      }

...
export function ZoneEditorMap({ center, zones, onZoneDrawn, onZoneEdited }: ZoneEditorMapProps) {
  ...
  const onMapLoad = useCallback((mapInstance: google.maps.Map) => {
    setMap(mapInstance);
    // Esperar un momento para asegurar que el mapa está completamente inicializado
  ...
  // Verificar que la libreria de Drawing este disponible
  const isDrawingAvailable = typeof google !== 'undefined' &&
    google.maps &&
    google.maps.drawing &&
  ...
  return (
    ...
    >
      {isMapReady && isDrawingAvailable && (
       ...
       {zones
        .filter((zone) => zone.path && Array.isArray(zone.path) && zone.path.length >= 3)
        .map((zone) => (
        <PolygonF
          key={zone.id}
          paths={zone.path}

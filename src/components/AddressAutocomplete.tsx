...
interface AddressAutocompleteProps {
  ...
  required?: boolean;
  id?: string;
  skipInitialSearch?: boolean; // Evitar búsqueda automática si ya viene pre-llenado
}
...
export function AddressAutocomplete({
  ...
  const [userHasTyped, setUserHasTyped] = useState(false); // Solo buscar si el usuario escribió
  const initialValueRef = React.useRef(value); // Guardar valor inicial

  // Debounce para búsqueda - solo si el usuario escribió manualmente
  useEffect(() => {
    // Si el valor es igual al inicial y skipInitialSearch está activo, no buscar
    if (skipInitialSearch && value === initialValueRef.current && !userHasTyped) {
      return;
    }

    if (value.length < 3 || !userHasTyped) {
      setSuggestions([]);
      return;
    }
  ...
  }, [value, userHasTyped, skipInitialSearch]);
    ...
    <div className="space-y-2 relative">
      ...
      <div className="relative">
        ...
        <Input
          ...
          value={value}
          onChange={(e) => {
            setUserHasTyped(true); // Marcar que el usuario escribió

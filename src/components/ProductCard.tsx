...
import type { DrinkOption, Product, Vendor, ProductVariant } from "@/lib/placeholder-data";
...
export function ProductCard({ product }: {product: Product}) {
  ...
  // Obtener la variante por defecto o la primera
  const defaultVariant = hasVariants ? (product.variants?.find(v => v.isDefault) || product.variants?.[0]) : undefined;
  ...
  // Si tiene variantes O tiene opciones, necesita el diálogo
  const needsOptionsDialog = hasVariants || hasOptions;
  ...
  const handleAddWithOptions = () => {
    ...
    const variantText = variantToUse ? ` (${variantToUse.name})` : '';
    toast({
        title: "¡Añadido al carrito!",
        description: `${product.name}${variantText} ha sido añadido con tus opciones.`,
    });
  ...
  const renderButton = () => {
    ...
    if (needsOptionsDialog) {
      // Calcular precio de la variante seleccionada o default para mostrar en el botón
                ...
                <div className="space-y-3 sm:space-y-4 py-3 sm:py-4">
                    ...
                    {hasVariants && product.variants && (
                        <div className="space-y-2">
                            <Label className="text-xs sm:text-sm font-semibold">Elige el tamaño</Label>
                            <RadioGroup
                                value={(selectedVariant || defaultVariant)?.id}
                                ...
                                {product.variants.map((variant) => (
                                    <div key={variant.id} className="relative">
                                        ...
                                        >
                                            ...
                                            <span className="text-sm font-bold text-primary">{settings.currencySymbol}{variant.price.toFixed(2)}</span>
                                        </Label>
                                    </div>
                                ))}
                            </RadioGroup>
  ...
  // Si tiene variantes, mostrar precio desde el mínimo
  if (hasVariants && product.variants && product.variants.length > 0) {
    ...
    if (minPrice !== maxPrice) {
      priceRangeText = `Desde`;
    }
  } else if (hasSpecificOfferPrice && product.offerPrice) {

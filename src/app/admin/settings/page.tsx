...
function AdminSettingsPageContent() {
  ...
  // Estados para trackear archivos originales antes de subir al servidor
    ...
    const { name, value } = e.target;
     setSettings(prev => ({
        ...
        paymentMethods: {
            ...prev.paymentMethods,
            gateway: {
                provider: prev.paymentMethods.gateway?.provider || 'none',
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Función helper para subir imagen al servidor
  ...
  const handleSaveQrPayment = async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      // Subir imagen al servidor si hay un archivo seleccionado
      let qrImageUrl = '';
      if (qrImageFile) {
          try {
              setIsUploading(true);
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 border rounded-md">
                            ...
                            <Input id="shipping-baseRadiusKm" name="baseRadiusKm" type="number" step="0.1" value={settings.shipping?.baseRadiusKm ?? 3} onChange={handleShippingChange} />
                            ...
                            <Input id="shipping-baseFee" name="baseFee" type="number" step="0.01" value={settings.shipping?.baseFee ?? 5} onChange={handleShippingChange} />
                            ...
                            <Input id="shipping-feePerKm" name="feePerKm" type="number" step="0.01" value={settings.shipping?.feePerKm ?? 1.5} onChange={handleShippingChange} />
                            ...
                            <TabsContent value="gateways" className="mt-6">
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between p-4 border rounded-md">
                                        ...
                                        <Switch
                                            id="cod-enabled"
                                            checked={settings.paymentMethods?.cashOnDeliveryEnabled ?? false}
                                    ...
                                    <div className="flex items-center justify-between p-4 border rounded-md">
                                        ...
                                        <Switch
                                            id="gateway-enabled"
                                            checked={settings.paymentMethods?.gateway?.enabled ?? false}
                                            onCheckedChange={(checked) => setSettings(prev => ({...prev, paymentMethods: {...prev.paymentMethods, gateway: {provider: prev.paymentMethods.gateway?.provider || 'none', ...prev.paymentMethods.gateway, enabled: checked}}}) )}
                                        />
                                    </div>
                                    ...
                                    <div className="p-4 border rounded-md space-y-4">
                                        <div className="space-y-2">
                                            ...
                                            <Select
                                                ...
                                                onValueChange={(value: PaymentGatewaySettings['provider']) => setSettings(prev => ({...prev, paymentMethods: {...prev.paymentMethods, gateway: {provider: value, ...prev.paymentMethods.gateway}}}) )}
                                            >

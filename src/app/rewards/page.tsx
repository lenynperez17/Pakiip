...
async function getUserCoinsFromAPI(userId: string) {
  ...
  throw new Error(data.error || "Error obteniendo monedas");
}

async function getRewardsFromAPI() {
  const res = await fetch("/api/data/rewards");
  const data = await res.json();
  if (data.success) {
    return data.data;
  }
  throw new Error(data.error || "Error obteniendo recompensas");
}

async function addCoinTransactionAPI(userId: string, transaction: Omit<CoinTransaction, 'id'>) {
  const res = await fetch("/api/data/coins", {
    method: "POST",
  ...
  throw new Error(data.error || "Error agregando transaccion");
}

async function saveUserCoinsAPI(userId: string, coins: number) {
  const res = await fetch("/api/data/coins", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
  ...
  throw new Error(data.error || "Error actualizando monedas");
}
...
function RewardsPageContent() {
  ...
  const loadUserData = async () => {
    ...
    setIsLoading(true);
    try {
      const userData = await getUserCoinsFromAPI(currentUser.id);
      if (userData) {
  ...
  const loadRewards = async () => {
    try {
      const rewardsData = await getRewardsFromAPI();
  ...
  const redeemReward = async (reward: Reward) => {
    ...
    try {
      // Crear transacción (la API actualiza el balance automaticamente)
      ...
      // Guardar en la API
      const result = await addCoinTransactionAPI(currentUser.id, transaction);

      // Actualizar estado local con el nuevo balance de la API
      if (result.newBalance !== undefined) {
        setUserCoins(result.newBalance);
      } else {
        setUserCoins(userCoins - reward.coinsCost);
      }

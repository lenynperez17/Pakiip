...
import type { Order, Vendor, AppSettings, DeliveryDriver, City, Coordinate, DeliveryZone } from './placeholder-data';
import type { EnrichedCartItem } from '@/hooks/use-cart';
...
export function calculateCartTotals(
    ...
    const subtotal = cartItems.reduce((acc, item) => {
        ...
        const variantPrice = (item.options as any)?.variant?.price;
        const price = variantPrice ?? item.product.offerPrice ?? item.product.price;
        let optionsPrice = 0;
    ...
    if (customerLocation) {
        ...
        if (!zoneFound) {
            ...
            for (const vendor of vendorsInCart) {
                // Verificar que el vendor tenga coordenadas válidas (no 0,0)
                if (vendor.coordinates && (vendor.coordinates.lat !== 0 || vendor.coordinates.lng !== 0)) {
            ...
            if (maxDistance > 0) {
                if (maxDistance <= baseRadiusKm) {
                    ...
                    const extraDistance = maxDistance - baseRadiusKm;
                    shipping = baseFee + (extraDistance * feePerKm);
                }
            } else {
    ...
    if (settings?.taxType === 'gravada') {
        tax = totalCommission * ((settings.taxRate || 0) / 100);
    }

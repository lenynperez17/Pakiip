...
// Variantes de producto (tamaños, presentaciones, etc.)
export type ProductVariant = {
  id: string;
  name: string; // Ej: "Personal", "Mediana", "Familiar", "Extra Grande"
  price: number;
  costPrice: number;
  stock?: number; // Stock específico por variante (opcional)
  isDefault?: boolean; // Variante seleccionada por defecto
}
...
export type Product = {
  ...
  // Sistema de variantes (tamaños, presentaciones)
  hasVariants?: boolean; // Activar sistema de variantes
...
export type Vendor = {
  ...
  qrImageUrl?: string; // URL de imagen - solo si paymentMethod === 'qr'
  qrPaymentName?: string; // Nombre del método QR (Yape, Plin, etc.)
  qrPaymentImageUrl?: string; // URL de imagen de QR de pago (Yape, Plin, etc.)
  description: string;
  category: string;
  imageUrl: string; // Logo
  logoUrl?: string; // Alias para imageUrl
  bannerUrl?: string; // Banner
  address: string; // Full address of the vendor
  location: string; // City name
  city?: string; // Alias para location
  sector?: string; // Sector/zona
...
// Configuración de usuario guardada en el servidor para persistencia
...
export type DeliveryDriver = {
    ...
    deliveries?: number; // Número de entregas completadas
};
...
export type Order = {
    id: string;
    date: string;
    customerId?: string; // ID del usuario cliente
    customerName: string;
    ...
    items: OrderItem[];
    driverId?: string;
    visibleToDriverId?: string; // ID del driver que puede ver este pedido
...
export type AppData = {
  ...
  // Propiedades opcionales a nivel superior (duplicadas desde appSettings para conveniencia)
  promotionalBanners?: PromotionalBanner[];
  announcementBanners?: PromotionalBanner[];
  bankAccounts?: BankAccount[];
  qrPayments?: QRPayment[];
